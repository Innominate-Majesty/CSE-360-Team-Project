...
public class ControllerFirstAdmin {
	...
	*/
	...
	protected static void doSetupAdmin(Stage ps, int r) {
		...
		// Run the username recognizer and get its output.
		String userNameResult = userNameRecognizerTestbed.UserNameRecognizer.checkForValidUserName(adminUsername);
		...
		if ((adminPassword1.compareTo(adminPassword2) == 0) &&
			...
			// The username is NOT valid, so clear the username, provide the error message from
			// the recognizer, and clear the message as soon as the first character is typed.
			ViewFirstAdmin.text_AdminUsername.setText("");
			ViewFirstAdmin.label_UsernameInvalid.setText(userNameResult);
		}
		else {
