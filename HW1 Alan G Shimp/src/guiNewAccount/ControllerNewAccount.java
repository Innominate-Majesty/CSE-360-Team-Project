...
public class ControllerNewAccount {
	...
	*/
	...
	protected static void doCreateUser() {
		...
		// Run the username recognizer and store the output.
		String userNameResult = 
				userNameRecognizerTestbed.UserNameRecognizer.checkForValidUserName(username);
		...
		else if (userNameResult.compareTo("") != 0) {
			// The username is NOT valid, so provide the appropriate error message, clear the
			// username, and clear the message as soon as the first character is typed.
			ViewNewAccount.text_Username.setText("");
			
			if (userNameResult.contains("empty")) {
				ViewNewAccount.alertUsernameInputEmpty.showAndWait();
			}
			else if (userNameResult.contains("A-Z ")) {
				ViewNewAccount.alertUsernameFirstCharIsInvalid.showAndWait();
			}
			else if (userNameResult.contains("period")) {
				ViewNewAccount.alertUsernamePunctuationError.showAndWait();
			}
			else if (userNameResult.contains("0-9")) {
				ViewNewAccount.alertUsernameCharIsInvalid.showAndWait();
			}
			else if (userNameResult.contains("4")) {
				ViewNewAccount.alertUsernameTooShort.showAndWait();
			}
			else if (userNameResult.contains("32")) {
				ViewNewAccount.alertUsernameTooLong.showAndWait();
			}
		}
		else {
