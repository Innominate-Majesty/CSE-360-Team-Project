...
public class UserNameRecognizer {
	...
	public static String checkForValidUserName(String input) {
		...
		finalState = false;					// State 0 is not the final state
		...
		while (running) {
			...
			switch (state) {
				case 0: 
					...
					// A-Z, a-z -> State 1
					// deleted 0-9 according to new FSM
				...
				case 1: 
					...
					// & -> State 2
							...
							currentChar == '_' ||
							currentChar == '-' ||
							currentChar == '&') {  // Check for separators
						nextState = 2;
					...
					// changed from 16 to 32 (the FSM in the HW1 details shows this as 32 is too long)
					if (userNameSize > 32)
						running = false;
					break;			
					
				case 2: 
					// State 2 deals with a character after a . _ - or & in the name.
					...
					// The execution of this state is finished
					// If the size is larger than 32, the loop must stop
					// Changed from 16 to 32 (the FSM in HW1 on Canvas shows 32)
					if (userNameSize > 32)
						running = false;
					break;			
			}
		...
		case 2:
			...
			if (currentCharNdx >= input.length()) {
				...
				userNameRecognizerErrorMessage +=
					"A UserName character after a . _ - or & must be A-Z, a-z, 0-9.\n";
			}
			return userNameRecognizerErrorMessage;
